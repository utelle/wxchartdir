...
IMPLEMENT_DYNAMIC_CLASS( CrossHair, wxDialog )

// CrossHair event table definition

BEGIN_EVENT_TABLE( CrossHair, wxDialog )
...
{    
  ...
  m_chartViewer = new wxChartViewer( itemDialog1, ID_CHARTVIEWER, wxDefaultPosition, FromDIP(wxSize(520,490)));
...
CrossHair::DrawChart(wxChartViewer* viewer)
{
  ...
  scatterLayer->setHTMLImageMap("", "", "title='<*cdml*>Concentration: {x} g/liter<*br*>Conductivity: {value} W/K'");
  ...
  trendLayer->setHTMLImageMap("", "", "title='<*cdml*>Trend Line: y = {slope|P4} x + {intercept|P4}'");
