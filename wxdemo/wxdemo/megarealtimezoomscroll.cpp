/*
** Name:        megarealtimezoomscroll.cpp
...
/*
 * MegaRealTimeZoomScroll type definition
 */

IMPLEMENT_CLASS(MegaRealTimeZoomScroll, wxDialog )

/*
 * MegaRealTimeZoomScroll event table definition
 */

BEGIN_EVENT_TABLE(MegaRealTimeZoomScroll, wxDialog )

  EVT_TOGGLEBUTTON( ID_POINTER, MegaRealTimeZoomScroll::OnPointerClick )
  EVT_TOGGLEBUTTON( wxID_ZOOM_IN, MegaRealTimeZoomScroll::OnZoomInClick )
  EVT_TOGGLEBUTTON( wxID_ZOOM_OUT, MegaRealTimeZoomScroll::OnZoomOutClick )
  EVT_BUTTON(ID_START, MegaRealTimeZoomScroll::OnPlotChart)
  EVT_COMMAND_SCROLL(ID_SCROLLBAR, MegaRealTimeZoomScroll::OnScrollbarUpdated)

  EVT_TIMER(ID_UPDATE_TIMER, MegaRealTimeZoomScroll::OnChartUpdateTimer)

  EVT_CHARTVIEWER_MOUSEMOVE_PLOTAREA(ID_CHARTVIEWER, MegaRealTimeZoomScroll::OnMouseMovePlotArea)
  EVT_CHARTVIEWER_VIEWPORT_CHANGED(ID_CHARTVIEWER, MegaRealTimeZoomScroll::OnViewPortChanged)
  EVT_CHARTVIEWER_MOUSEWHEEL(ID_CHARTVIEWER, MegaRealTimeZoomScroll::OnMouseWheelChart)

END_EVENT_TABLE()

/*
 * MegaRealTimeZoomScroll constructors
 */
...
MegaRealTimeZoomScroll::MegaRealTimeZoomScroll( wxWindow* parent, wxWindowID id, const wxString& caption, const wxPoint& pos, const wxSize& size, long style )
  : m_realTimeData(999)
{
  Init();
...
/*
 * MegaRealTimeZoomScroll creator
 */

bool
MegaRealTimeZoomScroll::Create( wxWindow* parent, wxWindowID id, const wxString& caption, const wxPoint& pos, const wxSize& size, long style )
{
...
/*
 * MegaRealTimeZoomScroll destructor
 */
...
void
MegaRealTimeZoomScroll::CreateControls()
{    
  SetIcon(wxICON(mondrian));
  MegaRealTimeZoomScroll* itemFrame1 = this;
  ...
  itemBoxSizer3->Add(m_startButton, 0, wxRIGHT | wxALL, FromDIP(3));
...
// Load the data
void
MegaRealTimeZoomScroll::LoadData()
{
  ...
  threads[0] = new RanSeriesTask(109, &m_dataSeriesA[0], initialSize);
  threads[1] = new RanSeriesTask(110, &m_dataSeriesB[0], initialSize);
  threads[2] = new RanSeriesTask(111, &m_dataSeriesC[0], initialSize);
  for (int j = 0; j < 3; ++j)
  {
    threads[j]->Run();
  }
...
bool
MegaRealTimeZoomScroll::getRealTimeData()
{
  wxDateTime now = wxDateTime::UNow();
  wxTimeSpan dt = now.Subtract(m_lastUpdateTime);
  wxLongLong milliseconds = dt.GetMilliseconds();
  m_lastUpdateTime = now;
  int pointCount = milliseconds.GetValue();
  ...
  m_startButton->SetLabel(wxString::Format("Point Count = %d x 3", m_currentIndex));

  return pointCount > 0;
}

void MegaRealTimeZoomScroll::updateAxisScale(wxChartViewer* viewer)
{
  //
...
void
MegaRealTimeZoomScroll::InitChartViewer(wxChartViewer* viewer)
{
  if (m_currentIndex > 0)
  {
...
void MegaRealTimeZoomScroll::OnViewPortChanged(wxCommandEvent& event)
{
...
void
MegaRealTimeZoomScroll::UpdateControls(wxChartViewer* viewer)
{
...
void
MegaRealTimeZoomScroll::DrawChart(wxChartViewer* viewer)
{
...
void
MegaRealTimeZoomScroll::OnMouseUsageChanged(int mouseUsage)
{
...
void
MegaRealTimeZoomScroll::OnScrollBarChanged(int value)
{
...
void
MegaRealTimeZoomScroll::OnMouseWheelChart(wxCommandEvent& event)
{
...
void
MegaRealTimeZoomScroll::OnMouseMovePlotArea(wxCommandEvent& event)
{
  if (!m_hasFinishedInitialization)
    return;
...
void
MegaRealTimeZoomScroll::TrackLineLabel(XYChart* c, int mouseX)
{
...
wxBitmap
MegaRealTimeZoomScroll::GetBitmapResource( const wxString& name )
{
  // Bitmap retrieval
  wxImage image;
...
void
MegaRealTimeZoomScroll::OnPointerClick( wxCommandEvent& event )
{
  m_pointerButton->SetValue(true);
  m_zoominButton->SetValue(false);
  m_zoomoutButton->SetValue(false);
...
void
MegaRealTimeZoomScroll::OnZoomInClick( wxCommandEvent& event )
{
...
void
MegaRealTimeZoomScroll::OnZoomOutClick( wxCommandEvent& event )
{
  m_pointerButton->SetValue(false);
  m_zoominButton->SetValue(false);
...
void
MegaRealTimeZoomScroll::OnPlotChart(wxCommandEvent& event)
{
  ...
  wxThread* threadA = new FastSeriesTask(m_fastData, "mA", &m_dataSeriesA[0], m_currentIndex);
  threadA->Run();

  wxThread* threadB = new FastSeriesTask(m_fastData, "mB", &m_dataSeriesB[0], m_currentIndex);
  threadB->Run();
  ...
  m_lastUpdateTime = wxDateTime::UNow();
...
void
MegaRealTimeZoomScroll::OnChartUpdateTimer(wxTimerEvent& event)
{
  if (getRealTimeData())
  {
...
void
MegaRealTimeZoomScroll::OnScrollbarUpdated(wxScrollEvent& event)
{
